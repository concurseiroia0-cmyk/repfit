...
const PERIOD_KEYS = [
  { value: '30' as const, labelKey: '30 dias' },
  { value: '90' as const, labelKey: '3 meses' },
  { value: '180' as const, labelKey: '6 meses' },
  { value: 'all' as const, labelKey: 'Tudo' },
];
...
export function EvolutionPage() {
  ...
  const freqData = useMemo(() => {
    ...
    for (const w of workouts) {
      ...
      const week = format(parseLocalDate(w.date), 'dd/MM', { locale: dateLocale() });
  ...
  return (
    <div>
      <h1 className="mb-1 text-xl font-extrabold text-slate-900 dark:text-white">{t('Evolução')}</h1>
      <p className="mb-4 text-sm text-slate-500 dark:text-slate-400">
        {t('Acompanhe a progressão dos seus exercícios ao longo do tempo.')}
      </p>
      ...
      <Card className="mb-4 p-5">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <Field label={t('Exercício')}>
            <Select value={exercise} onChange={(e) => setExercise(e.target.value)} aria-label={t('Selecionar exercício')}>
          ...
          <Field label={t('Período')}>
            <SegmentedControl
              options={PERIOD_KEYS.map((p) => ({ value: p.value, label: t(p.labelKey) }))}
              value={period}
              onChange={setPeriod}
              ariaLabel={t('Filtrar por período')}
            />
          </Field>
        </div>
      </Card>
      ...
      <Card className="mb-4 p-5">
        <h2 className="mb-3 flex items-center gap-2 text-base font-bold text-slate-900 dark:text-white">
          <Trophy className="h-5 w-5 text-amber-400" /> {t('Recordes pessoais')}
        </h2>
        {records.length === 0 ? (
          <p className="text-sm text-slate-500 dark:text-slate-400">
            {t('Nenhum recorde ainda. Bata seus próprios limites!')}
          </p>
        ) : (
            ...
            {records.slice(0, 6).map((r) => (
              <div key={r.key} className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800/60">
                ...
                <p className="mt-0.5 text-lg font-extrabold text-slate-900 dark:text-white">
                  {r.unit === 'kg' ? formatWeight(r.value, unit) : `${formatNumber(r.value)} ${r.unit === 'dias' ? t('dias') : t('reps')}`}
                </p>
      ...
      {exerciseNames.length === 0 ? (
        <EmptyState
          ...
          description={t('Registre alguns treinos para ver gráficos e progressão aqui.')}
        />
      ) : (
        <>
          ...
          <Card className="mb-4 p-5">
            <h2 className="mb-2 text-sm font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500">
              {t('Progressão — {{exercicio}}', { exercicio: exercise })}
            </h2>
            {history.length === 0 ? (
              <p className="py-4 text-sm text-slate-500 dark:text-slate-400">
                {t('Sem registros de <b>{{exercicio}}</b> no período selecionado.', { exercicio: exercise })}
              </p>
            ) : (
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
            <ChartCard title={t('Evolução da carga')} subtitle={t('Carga máxima por treino')} empty={weightData.length === 0 ? t('Sem cargas registradas.') : undefined}>
              <LineChart data={weightData} unit={unit} />
            </ChartCard>
            <ChartCard title={t('Evolução das repetições')} subtitle={t('Máximo de repetições por treino')} empty={repsData.length === 0 ? t('Sem repetições registradas.') : undefined}>
              <LineChart data={repsData} unit={t('reps')} color="#0ea5e9" />
            </ChartCard>
            <ChartCard title={t('Volume do exercício')} subtitle={t('Soma do volume por treino')} empty={volData.length === 0 ? t('Sem volume registrado.') : undefined}>
              <BarChart data={volData} unit={unit} />
            </ChartCard>
            <ChartCard title={t('Frequência de treinos')} subtitle={t('Treinos por semana')} empty={freqData.length === 0 ? t('Sem treinos no período.') : undefined}>
              <BarChart data={freqData} unit={t('treinos')} />
            </ChartCard>
            <ChartCard
              title={t('Tempo de descanso por treino')}
              subtitle={t('Descanso total registrado no formulário (minutos)')}
              empty={restData.length === 0 ? t('Sem descanso registrado no período.') : undefined}
            >
              <BarChart data={restData} unit="min" />
            </ChartCard>
            <ChartCard title={t('Esforço médio')} subtitle={t('Média da escala 1–6 (1 = mais difícil)')} empty={effortData.length === 0 ? t('Sem esforço registrado.') : undefined}>
          ...
          {exerciseNames.length >= 2 && (
            <Card className="mt-6 p-5">
              <h2 className="mb-3 flex items-center gap-2 text-base font-bold text-slate-900 dark:text-white">
                <GitCompare className="h-5 w-5 text-sky-500" /> {t('Comparar dois exercícios')}
              </h2>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <Field label={t('Exercício A')}>
                  <Select value={compareA} onChange={(e) => setCompareA(e.target.value)} aria-label={t('Exercício A para comparação')}>
                    {exerciseNames.map((n) => (
                      <option key={n} value={n}>
                        {n}
                      </option>
                ...
                <Field label={t('Exercício B')}>
                  <Select value={compareB} onChange={(e) => setCompareB(e.target.value)} aria-label={t('Exercício B para comparação')}>
                    {exerciseNames.map((n) => (
                      <option key={n} value={n}>
                        {n}
                      </option>
              ...
              {compareA && compareB && compareA !== compareB ? (
                <>
                  ...
                  <div className="mt-4 grid grid-cols-1 gap-4 lg:grid-cols-2">
                    <ChartCard
                      title={t('Evolução da carga')}
                      subtitle={t('Carga máxima por ocorrência (1º, 2º… treino com o exercício)')}
                      empty={weightA.length + weightB.length === 0 ? t('Sem cargas registradas no período.') : undefined}
                    >
                      <MultiLineChart
                        series={[
                    ...
                    <ChartCard
                      title={t('Volume por treino')}
                      subtitle={t('Soma do volume do exercício por ocorrência')}
                      empty={volA.length + volB.length === 0 ? t('Sem volume registrado no período.') : undefined}
                    >
                      <MultiLineChart
                        series={[
                  ...
                  <p className="mt-3 text-xs text-slate-400 dark:text-slate-500">
                    {t('O eixo X mostra a ocorrência do exercício (1º treino, 2º treino…), alinhando as progressões mesmo quando os exercícios foram feitos em dias diferentes.')}
                  </p>
                </>
              ) : (
                <p className="mt-3 text-sm text-slate-500 dark:text-slate-400">
                  {t('Escolha dois exercícios <b>diferentes</b> para comparar carga e volume lado a lado.')}
                </p>
              )}
            </Card>
          )}
    ...
    <Card className="mb-4 p-5">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="flex items-center gap-2 text-sm font-bold text-slate-900 dark:text-white">
          <TrendIcon className={`h-5 w-5 ${trendColor}`} /> {t('Progressão da carga')}
        </h2>
        {delta !== 0 && (
      ...
      <div className="mt-3 grid grid-cols-3 gap-2 text-center">
        <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800/60">
          ...
          <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t('primeira')}</div>
        </div>
        <div className="rounded-xl bg-amber-50 p-3 dark:bg-amber-400/10">
          ...
          <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t('última')}</div>
        </div>
        <div className="rounded-xl bg-slate-50 p-3 dark:bg-slate-800/60">
          <div className="text-base font-extrabold text-slate-900 dark:text-white">{formatWeight(maior, unit)}</div>
          <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t('maior carga')}</div>
        </div>
      </div>
      <p className="mt-2 text-xs text-slate-400 dark:text-slate-500">
        {t('{{n}} treino(s) com este exercício no período selecionado.', { n: loaded.length })}
      </p>
    </Card>
  );
}
    ...
    <div className="rounded-xl bg-slate-50 p-3.5 dark:bg-slate-800/60">
      ...
      <div className="mt-2 grid grid-cols-3 gap-2 text-center">
        ...
        <div>
          ...
          <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t('última carga')}</div>
        </div>
        <div>
          <div className="text-sm font-extrabold text-slate-900 dark:text-white">
            {stats.volumeTotal > 0 ? `${formatNumber(stats.volumeTotal)} ${unit}` : '—'}
          </div>
          <div className="text-[10px] font-semibold uppercase tracking-wide text-slate-400">{t('volume total')}</div>
        </div>
      </div>
    </div>
  );
