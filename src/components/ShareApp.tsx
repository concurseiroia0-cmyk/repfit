...
export function ShareAppModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function copy() {
    try {
      ...
      setCopied(true);
      push(t('Link copiado!'), 'success');
      window.setTimeout(() => setCopied(false), 2000);
    } catch {
      push(t('Não foi possível copiar o link automaticamente.'), 'error');
    }
  }
  ...
  async function handleShare() {
    ...
    if (typeof nav.share === 'function') {
      try {
        await nav.share({ title: 'RepFit', text: t('Registre e acompanhe seus treinos — 100% offline e privado.'), url });
        return;
      } catch {
  ...
  return (
    <Modal open={open} onClose={onClose} title={t('Instalar no celular')} size="sm">
      <div className="space-y-4 text-sm">
        <p className="text-slate-600 dark:text-slate-300">
          {t('Abra a câmera do celular e aponte para o <b>QR code</b> para abrir o RepFit, ou compartilhe o link com alguém.')}
        </p>
        ...
        {local && (
          <div className="flex items-start gap-2.5 rounded-xl border border-amber-300/60 bg-amber-50 p-3.5 text-xs leading-relaxed text-amber-800 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-300">
            ...
            <span>
              <b>{t('Atenção:')}</b> {t('este endereço é <b>local</b> (funciona só neste computador). Para abrir no celular, publique o app em um endereço acessível pela internet ou acesse pelo IP da sua rede Wi-Fi.')}
            </span>
          </div>
        )}
        ...
        <div className="flex flex-col gap-2 sm:flex-row">
          <Button onClick={() => void handleShare()} full className="sm:flex-1">
            <Share2 className="h-4 w-4" /> {t('Compartilhar link')}
          </Button>
        ...
        <p className="flex items-start gap-2 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
          ...
          {t('Depois de abrir no celular, use “Adicionar à Tela de Início” (iOS) ou instalar (Android) para ter o app instalado. Os dados ficam salvos apenas em cada dispositivo.')}
        </p>
        <div className="rounded-xl border border-sky-200 bg-sky-50 p-3.5 text-xs leading-relaxed text-sky-800 dark:border-sky-400/30 dark:bg-sky-400/10 dark:text-sky-300">
          <p className="font-bold">{t('Quer ver seus treinos no celular? Conecte sua conta')}</p>
          <p className="mt-1">{t('O app instalado começa vazio. Para trazer o histórico do navegador: no aparelho onde sua conta já está logada, abra <b>Configurações → Conectar aplicativo</b>, pegue o código de 6 dígitos e digite-o no celular em <b>“Já tenho uma conta”</b> — seus treinos e medidas sincronizam pela nuvem automaticamente.')}</p>
        </div>
      </div>
    </Modal>
  );
