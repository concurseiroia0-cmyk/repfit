...
const MODE_LABEL_KEYS: Record<(typeof MODES)[number], string> = {
  academia: 'Academia',
...
export function WorkoutForm({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Dados gerais */}
      <Card>
        <CardHeader title={t('Dados do treino')} />
        <div className="space-y-4 px-5 pb-5">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field label={t('Data')}>
              <Input type="date" value={form.date} max={todayString()} onChange={(e) => update({ date: e.target.value })} />
            </Field>
            <Field label={t('Dia da semana')} hint={form.date ? formatDate(form.date) : undefined}>
          ...
          <Field label={t('Nome do treino')}>
            <Input
              type="text"
              value={form.name}
              onChange={(e) => update({ name: e.target.value })}
              placeholder={t('Ex.: Treino A, Peito e Tríceps')}
            />
          </Field>
          <Field label={t('Tipo de treino')}>
            <div className="flex flex-wrap gap-1.5">
              {WORKOUT_TYPES.map((tKey) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    aria-pressed={active}
                    onClick={() => update({ type: active ? '' : tKey })}
                    className={cn(
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field label={t('Duração (minutos)')} hint={t('Opcional')}>
              <Input
                type="text"
                inputMode="numeric"
            ...
            <Field label={t('Observação geral')} hint={t('Opcional')}>
              <Input
                type="text"
                value={form.notes}
                onChange={(e) => update({ notes: e.target.value })}
                placeholder={t('Como foi o treino?')}
              />
            </Field>
          </div>
        </div>
      ...
      {/* Descanso (opcional) */}
      <Card>
        <CardHeader title={t('Descanso')} subtitle={t('Opcional — houve descanso entre as séries?')} />
      ...
      <Card>
        <CardHeader
          title={t('Exercícios')}
          subtitle={`${form.exercises.length} ${t('exercícios')} · ${setCount} ${t('séries')}`}
          action={
          ...
          {quick.length > 0 && (
            <div className="mb-4">
              <p className="mb-1.5 flex items-center gap-1 text-xs font-bold uppercase tracking-wide text-slate-400 dark:text-slate-500">
                <Zap className="h-3.5 w-3.5" /> {t('Rápidos')}
              </p>
              <div className="flex flex-wrap gap-1.5">
                {quick.map((c) => (
                  <button
          ...
          {form.exercises.length === 0 ? (
            <div className="rounded-xl border-2 border-dashed border-slate-200 py-10 text-center dark:border-white/15">
              ...
              <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
                {t('Nenhum exercício ainda. Toque em <b>Adicionar</b> ou escolha um dos rápidos acima.')}
              </p>
            </div>
          ) : (
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        ...
        {onSubmitAndNew && (
          <Button full size="lg" variant="secondary" onClick={() => onSubmitAndNew()} disabled={saving} className="flex-1">
            {saving ? t('Salvando…') : t('Salvar e novo treino')}
          </Button>
        )}
      </div>
    </div>
